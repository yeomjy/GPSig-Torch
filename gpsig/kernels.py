    ...
    def __init__(
        ...
        if self.order != 1 and low_rank:
            raise NotImplementedError(
                "Higher-order algorithms not compatible with low-rank mode (yet)."
            )
        ...
        self.variances = Parameter(
            self._validate_signature_param(
                "variances", variances, num_levels + 1
            ),
        ...
        if num_lags is None:
            ...
            if not isinstance(num_lags, int) or num_lags < 0:
                raise ValueError(
                    "The variable num_lags most be a nonnegative integer or None."
                )
            else:
                ...
                if num_lags > 0:
                    ...
                    gamma = 1.0 / np.asarray(range(1, self.num_lags + 2))
                    gamma /= np.sum(gamma)
        ...
        if lengthscales is not None:
            lengthscales = self._validate_signature_param(
                "lengthscales", lengthscales, self.num_features
            )
...
class SignatureLinear(SignatureKernel):
    ...
    def __init__(self, input_dim, num_features, num_levels, **kwargs):
        SignatureKernel.__init__(
            self, input_dim, num_features, num_levels, **kwargs
        )
...
class SignaturePoly(SignatureKernel):
    ...
    def __init__(
        self, input_dim, num_features, num_levels, gamma=1, degree=3, **kwargs
    ):
        SignatureKernel.__init__(
            self, input_dim, num_features, num_levels, **kwargs
        )
        self.gamma = Parameter(
            gamma, transform=transforms.positive, dtype=settings.float_type
        )
        self.degree = Parameter(
            degree, dtype=settings.float_type, trainable=False
        )
    ...
    def _poly(self, X, X2=None):
        if X2 is None:
            return (
                tf.matmul(X, X, transpose_b=True) + self.gamma
            ) ** self.degree
        else:
            return (
                tf.matmul(X, X2, transpose_b=True) + self.gamma
            ) ** self.degree
...
class SignatureMix(SignatureKernel):
    ...
    def __init__(self, input_dim, num_features, num_levels, **kwargs):
        ...
        self.mixing = Parameter(
            0.5, transform=transforms.positive, dtype=settings.float_type
        )
    ...
    def _mix(self, X, X2=None):
        ...
        K = self.mixing * tf.exp(-ds / 2) + (1.0 - self.mixing) * inner
        return K
...
class SignatureSpectral(SignatureKernel):
    ...
    def __init__(
        self, input_dim, num_features, num_levels, family="gauss", Q=5, **kwargs
    ):
        SignatureKernel.__init__(
            self,
            input_dim,
        ...
        self.alpha = Parameter(
            np.exp(np.random.randn(Q)),
            transform=transforms.positive,
            dtype=settings.float_type,
        )
        self.omega = Parameter(
            np.exp(np.random.randn(Q, self.num_features)),
            transform=transforms.positive,
            dtype=settings.float_type,
        )
        self.gamma = Parameter(
    ...
    def _spectral(self, X, X2=None):
        if X2 is None:
            diff_tiled = tf.tile(
                (X[None, :, None, :] - X[None, None, :, :]), [self.Q, 1, 1, 1]
            )
        else:
            diff_tiled = tf.tile(
                (X[None, :, None, :] - X2[None, None, :, :]), [self.Q, 1, 1, 1]
            )
        ...
        if self.family == "exp":
            kern_term = tf.exp(
                -tf.sqrt(
                    tf.reduce_sum(
                        tf.square(diff_tiled * self.gamma[:, None, None, :]),
                        axis=-1,
                    )
                )
                / 2
            ...
            Q1 = tf.cast(
                tf.floor(tf.cast(Q, settings.float_type) / 2.0),
                settings.int_type,
            )
            Q2 = tf.cast(
                tf.ceil(tf.cast(Q, settings.float_type) / 2.0),
                settings.int_type,
            )
            square_dist = -tf.reduce_sum(
                tf.square(diff_tiled * self.gamma[:, None, None, :]), axis=-1
            )
            rbf_term = tf.exp(-1.0 * square_dist[:Q1] / 2)
            exp_term = tf.exp(-1.0 * tf.sqrt(square_dist[Q1:]) / 2)
        spectral_term = tf.cos(
            2.0
            * np.pi
            * tf.reduce_sum(diff_tiled * self.omega[:, None, None, :], axis=-1)
        )
        if self.family == "mixed":
            return tf.reduce_sum(
                rbf_term * spectral_term[:Q1] * self.alpha[:Q1, None, None],
                axis=0,
            ) + tf.reduce_sum(
                exp_term * spectral_term[Q1:] * self.alpha[Q1:, None, None],
                axis=0,
            )
        else:
            return tf.reduce_sum(
                kern_term * spectral_term * self.alpha[:, None, None], axis=0
            )
...
class SignatureMatern32(SignatureKernel):
    ...
    def _Matern32(self, X, X2=None):
        ...
        return (1.0 + np.sqrt(3.0) * r) * tf.exp(-np.sqrt(3.0) * r)
...
class SignatureMatern52(SignatureKernel):
    ...
    def _Matern52(self, X, X2=None):
        r = self._euclid_dist(X, X2)
        return (1.0 + np.sqrt(5.0) * r + 5.0 / 3.0 * tf.square(r)) * tf.exp(
            -np.sqrt(5.0) * r
        )
